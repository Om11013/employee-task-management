...
import type { Task, GetTasksQuery } from "../types";
import { useAuth } from "../../../hooks/useAuth";

const Tasks = () => {
  const { isAdmin } = useAuth();
  const [params, setParams] = useState<GetTasksQuery>({
    page: 1,
    limit: 10,
    sortBy: "createdAt",
    sortOrder: "desc",
  ...
  const handleSort = (sortBy: string, sortOrder: string) => {
    setParams((prev) => ({
      ...prev,
      sortBy: sortBy as GetTasksQuery["sortBy"],
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 space-y-4 sm:space-y-0">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            {isAdmin
              ? "Manage and oversee all company tasks."
      ...
      <TaskFilters
        ...
        currentAssignedEmployeeId={params.assignedEmployeeId}
      />
